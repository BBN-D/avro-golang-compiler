...
// Adapted from http://piumarta.com/software/memstream/memstream-0.1/memstream.c
...
func fmemopen(bytes []byte) *C.FILE {
	mode := C.CString("rb")
	defer C.free(unsafe.Pointer(mode))
	
	return C.fmemopen(unsafe.Pointer(&bytes[0]), C.size_t(len(bytes)), mode)
}

// MemStream is used to store avro results
type MemStream struct {
	Buffer []byte	// Only valid after calling GetBuffer!	
	
	file *C.FILE	// C file pointer
	buf *C.char
	buflen C.size_t
}

func openMemstream() *MemStream {
	m := new (MemStream)
	m.file = C.open_memstream(&m.buf, &m.buflen)
	if m.file == nil {
		return nil
	}
...
func (m *MemStream) GetBuffer() []byte {
	if m.buf == nil {
		return nil
	}
	
	// TODO: avoid the copy here
	m.Buffer = C.GoBytes(unsafe.Pointer(m.buf), C.int(m.buflen))
	return m.Buffer
}

func (this *MemStream) Close() {
	if this.buf != nil {
